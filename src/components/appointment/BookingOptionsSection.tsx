...
import { Phone, MapPin, Activity, Check, ArrowRight } from 'lucide-react';
    ...
    >
      ...
      <div className="p-6 md:p-8 flex-1 flex flex-col">
        ...
        <div className="flex items-center mb-4">
          <div className="mr-4 text-[#7BA7C2]">
            {icon}
          </div>
        ...
        <div className="flex-1">
          ...
          <ul className="space-y-2">
            {benefits.map((benefit, index) => (
              <li key={index} className="flex items-start min-h-[40px]">
  ...
  return (
    <section className="py-16 md:py-24 relative overflow-hidden">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 relative z-10">
        ...
        {/* Options Cards */}
        <div className={`grid md:grid-cols-3 gap-6 md:gap-8 transition-all duration-1000 delay-300 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          {/* Phone Consultation Option */}
          <OptionCard
            title={t('bookingSection.phoneOption.title')}
            description={t('bookingSection.phoneOption.description')}
            benefits={t('bookingSection.phoneOption.benefits', { returnObjects: true }) as string[]}
            ...
            buttonText={t('bookingSection.chooseButton')}
          />
          
          {/* In-Person Consultation Option */}
          <OptionCard
            title={t('bookingSection.personalOption.title')}
            description={t('bookingSection.personalOption.description')}
            benefits={t('bookingSection.personalOption.benefits', { returnObjects: true }) as string[]}
            icon={<MapPin strokeWidth={1.5} className="w-6 h-6" />}
            isHovered={hoveredOption === 'personal'}
            onHover={(isHovered) => handleHover('personal', isHovered)}
            calendlyUrl="https://calendly.com/dionhairclinic/beratungklinik"
            buttonText={t('bookingSection.chooseButton')}
          />
          
          {/* Mesotherapy Treatment Option */}
          <OptionCard
            title={t('bookingSection.mesotherapyOption.title')}
            description={t('bookingSection.mesotherapyOption.description')}
            benefits={t('bookingSection.mesotherapyOption.benefits', { returnObjects: true }) as string[]}
            icon={<Activity strokeWidth={1.5} className="w-6 h-6" />}
            isHovered={hoveredOption === 'mesotherapy'}
            onHover={(isHovered) => handleHover('mesotherapy', isHovered)}
            calendlyUrl="https://calendly.com/dionhairclinic/mesotherapie"
            buttonText={t('bookingSection.chooseButton')}
          />
        </div>
      </div>
