...
const FAQSection: React.FC = () => {
  const { t, i18n } = useTranslation(['home', 'common', 'hairTransplantation', 'beardTransplantation', 'eyebrowTransplantation', 'hairLossTherapy']);
  ...
  const faqCategories = ['hair', 'beard', 'eyebrows', 'therapy', 'general', 'costs', 'aftercare'];
  
  // Get FAQs from home namespace
  const homeFaqs: FAQ[] = Array.from(
    ...
    (_, index) => ({
      ...
      category: t(`faqSection.faqs.${index}.category`, { defaultValue: faqCategories[index % faqCategories.length] }),
      source: 'home'
    })
  );
  
  // Get FAQs from hair transplantation namespace
  const hairTransplantationFaqs: FAQ[] = Array.from(
    { length: (t('faqSection.faqs', { returnObjects: true, ns: 'hairTransplantation' }) as any[]).length },
    (_, index) => ({
      question: t(`faqSection.faqs.${index}.question`, { ns: 'hairTransplantation' }),
      answer: t(`faqSection.faqs.${index}.answer`, { ns: 'hairTransplantation' }),
      category: 'hair',
      source: 'hairTransplantation'
    })
  ).slice(0, 3); // Take only the first 3 FAQs
  
  // Get FAQs from beard transplantation namespace
  const beardTransplantationFaqs: FAQ[] = Array.from(
    { length: (t('faqSection.faqs', { returnObjects: true, ns: 'beardTransplantation' }) as any[]).length },
    (_, index) => ({
      question: t(`faqSection.faqs.${index}.question`, { ns: 'beardTransplantation' }),
      answer: t(`faqSection.faqs.${index}.answer`, { ns: 'beardTransplantation' }),
      category: 'beard',
      source: 'beardTransplantation'
    })
  ).slice(0, 3); // Take only the first 3 FAQs
  
  // Get FAQs from eyebrow transplantation namespace
  const eyebrowTransplantationFaqs: FAQ[] = Array.from(
    { length: (t('faqSection.faqs', { returnObjects: true, ns: 'eyebrowTransplantation' }) as any[]).length },
    (_, index) => ({
      question: t(`faqSection.faqs.${index}.question`, { ns: 'eyebrowTransplantation' }),
      answer: t(`faqSection.faqs.${index}.answer`, { ns: 'eyebrowTransplantation' }),
      category: 'eyebrows',
      source: 'eyebrowTransplantation'
    })
  ).slice(0, 3); // Take only the first 3 FAQs
  
  // Get FAQs from hair loss therapy namespace
  const hairLossTherapyFaqs: FAQ[] = Array.from(
    { length: (t('faqSection.faqs', { returnObjects: true, ns: 'hairLossTherapy' }) as any[]).length },
    (_, index) => ({
      question: t(`faqSection.faqs.${index}.question`, { ns: 'hairLossTherapy' }),
      answer: t(`faqSection.faqs.${index}.answer`, { ns: 'hairLossTherapy' }),
      category: 'therapy',
      source: 'hairLossTherapy'
    })
  ).slice(0, 3); // Take only the first 3 FAQs
      ...
      <div className="w-full max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-20">
          <div className="inline-block mb-4 relative">
            ...
            <h2 className={`${textStyle.sectionTitle} text-3xl md:text-4xl lg:text-5xl leading-tight md:leading-snug lg:leading-normal px-2 md:px-0 hyphens-auto break-words`} lang="de">{t('faqSection.title')}</h2>
